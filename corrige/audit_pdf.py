...
def header(c, auditor, n, n_common, truth):
    ...
    rules = [
        "Lisez le TEXTE de l'acte A (lien « ouvrir le texte »). Ne regardez JAMAIS le cadre « Relations entre documents » ou « Informations sur le document » d'EUR-Lex : ce cadre, c'est la base qu'on contrôle. S'y fier, c'est faire noter l'élève par lui-même.",
...
def fact_block(c, f, a, b, titles, common, y):
    rel, q, hint = Q[f["p"]]
    ...
    x = link(c, "ouvrir le texte (FR)", M + 14, y, eurlex(a["celex"], "FR")); c.setFont(FONT, 9.5); c.drawString(x + 4, y, "·"); link(c, "EN", x + 12, y, eurlex(a["celex"], "EN")); y -= 13
    y = wrap(c, f"B : {tb} — CELEX {b['celex']} · {b.get('date_document') or ''}", M, y, W - 2 * M)
    x = link(c, "ouvrir le texte (FR)", M + 14, y, eurlex(b["celex"], "FR")); c.setFont(FONT, 9.5); c.drawString(x + 4, y, "·"); link(c, "EN", x + 12, y, eurlex(b["celex"], "EN")); y -= 14
...
def main():
    ...
    for auditor, ids in rec["per_auditor"].items():
        ...
        for fid in ids:
            f = facts[fid]; y = fact_block(c, f, nodes[f["s"]], nodes[f["o"]], titles, common, y)
