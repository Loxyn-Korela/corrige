...
for r in $(seq 1 "$N"); do
  ...
  for wl in workloads/eurlex-laws.toml workloads/eurlex-labels.toml; do
    tag=$(basename "$wl" .toml)
    pg-repair-run -u neo4j --db-password "$NEO4J_LOCAL_PASSWORD" --disable-cache --commit repair --mark \
      -a SciPyWeightedILP "$C/$wl" > "$C/runs/seven/pgrepair-$tag-r$r.log" 2>&1
    grep -o "deleting [0-9]* edge(s), [0-9]* node(s), [0-9]* labels" "$C/runs/seven/pgrepair-$tag-r$r.log" | tail -1
  done
  cd "$C"; python3 -m corrige.neo4j_graph read runs/seven/graph.json "runs/seven/candidate-both-r$r.json" --name pgrepair
